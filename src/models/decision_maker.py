...
from src.prompts import INPUT_STR, INSTRUCTION, SYSTEM_MSG, TEMPLATE
...
class Decision_Model:
    ...
    async def predict(self, ticker, price, cash, share):
        ...
        trend_prediction = (
            "+" + str(trend_prediction_raw)
            if (trend_prediction_raw >= 0)
            else str(trend_prediction_raw)
        )
        ...
        return response.choices[0].message.content, lstm_output, bert_output
