...
class backetester:
    ...
    async def trade(self, ticker, date, external_price=None):
        ...
        response_raw, _, _ = await self.decision_maker.predict(
            ticker, price, cash, share
        )
        print(response_raw)
        ...
        return {"status": "success", "ticker": ticker, "decision": response}
    ...
    async def run_backtest(self, tickers, start_date, end_date):
        ...
        all_data = yf.download(
            tickers, start=start_date, end=end_date, group_by="column"
        )
        ...
        self.show_total_performance(
            start_cash, tickers, last_prices, bh_remaining_cash, buy_hold_shares
        )
        self.plot_trading_results(tickers, open_prices)

    def show_total_performance(
        self, start_cash, tickers, last_prices, bh_remaining_cash, bh_shares
    ):
        ...
        print("-" * 40)
        print(
            f"{'AI Strategy':<15} | {strategy_total:>10.2f} | {strategy_return:>7.2f}%"
        )
        print(f"{'Buy & Hold':<15} | {bh_total:>10.2f} | {bh_return:>7.2f}%")
        print("-" * 40)
        print(
            f"How AI performed better compared to BH: {strategy_return - bh_return:.2f}%p"
        )
        print("=" * 40)
    ...
    def get_full_status(self):
        ...
        return {"current_cash": cash, "portfolio": portfolio}
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        await tester.run_backtest(
            tickers=tickers, start_date="2025-02-01", end_date="2025-02-28"
        )
